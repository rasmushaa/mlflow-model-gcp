...
class RandomForestModel(BaseModel):
    """Model component that wraps a RandomForestClassifier from scikit-learn.

    This model can be used for classification tasks and supports the same parameters as sklearn's RandomForestClassifier.
    ...
    **kwargs
        Additional keyword arguments to pass to the RandomForestClassifier constructor.
    """
    ...
    def _fit_selected(self, X_selected, y):
        """Fit the RandomForestClassifier using the selected features and target variable."""
        self.__model.fit(X_selected, y)

    def _predict_selected(self, X_selected):
        """Predict class labels for the selected features using the fitted RandomForestClassifier."""
        return self.__model.predict(X_selected)

    def _predict_proba_selected(self, X_selected):
        """Predict class probabilities for the selected features using the fitted RandomForestClassifier."""
    ...
    def classes(self) -> list:
        ...
        if hasattr(self.__model, "classes_"):
            ...
            raise AttributeError(
                "The RandomForestModel has not been fitted yet, so classes are unavailable."
            )
