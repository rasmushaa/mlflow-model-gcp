import pytest
import numpy as np
from src.utils.ml.metrics import prediction_report_metrics, classification_report_metrics
...
def test_classification_report_metrics_multiclass():
    ...
    assert abs(metrics['precision.0'] - 0.5) < tolerance           # TP / (TP + FP) for only class 0
    assert abs(metrics['precision.1'] - 0.6666) < tolerance        # TP / (TP + FP) for only class 1
    assert abs(metrics['precision.2'] - 1.0) < tolerance           # TP / (TP + FP) for only class 2
    assert abs(metrics['recall.0'] - 1.0) < tolerance              # TP / (TP + FN) for only class 0
    assert abs(metrics['recall.1'] - 0.6666) < tolerance           # TP / (TP + FN) for only class 1
    assert abs(metrics['recall.2'] - 0.75) < tolerance             # TP / (TP + FN) for only class 2
    assert abs(metrics['f1.0'] - 0.66666) < tolerance              # 2 * (precision * recall) / (precision + recall) for only class 0
    assert abs(metrics['f1.1'] - 0.6666) < tolerance               # 2 * (precision * recall) / (precision + recall) for only class 1
    assert abs(metrics['f1.2'] - 0.8571) < tolerance               # 2 * (precision * recall) / (precision + recall) for only class 2
    ...
    assert abs(metrics['f1.micro'] - 0.75) < tolerance             # 2 * (precision_micro * recall_micro) / (precision_micro + recall_micro)
...
def test_prediction_report_metrics_multi_class():
    ...
    tolerance = 0.0001
    assert abs(metrics['roc_auc.0'] - 1.0) < tolerance          # AUC for only class 0 in One VS All binary classification
    assert abs(metrics['roc_auc.1'] - 0.8999) < tolerance       # AUC for only class 1 in One VS All binary classification
    assert abs(metrics['roc_auc.2'] - 1.0) < tolerance          # AUC for only class 2 in One VS All binary classification
    assert abs(metrics['roc_auc.macro'] - 0.9666) < tolerance   # (AUC_1 + AUC_2 + AUC_3) / 3
    assert abs(metrics['roc_auc.micro'] - 0.9687) < tolerance   # Overall AUC in One VS All fashion
...
def test_prediction_report_metrics_binary():
    ...
    tolerance = 0.0001
    assert abs(metrics['roc_auc'] - 0.9166) < tolerance # Normal AUC for binary classification
