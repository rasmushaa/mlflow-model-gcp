...
def plot_confusion_matrix(ax: plt.Axes, y_true: np.ndarray, y_pred: np.ndarray) -> plt.Axes:
...
def plot_roc_auc(ax: plt.Axes, fpr: dict, tpr: dict, roc_auc: dict) -> plt.Axes:
    ...
    if len(fpr) > 1:
        ...
        if 'micro' in fpr and 'macro' in fpr:
            ax.plot(fpr['micro'], tpr['micro'], label=f'Micro (AUC = {roc_auc["micro"]:.2f})', color='darkgreen', ls='--')
            ax.plot(fpr['macro'], tpr['macro'], label=f'Macro (AUC = {roc_auc["macro"]:.2f})', color='forestgreen', ls='--')
        ...
        for label in classes:
            ax.plot(fpr[label], tpr[label], label=f'{label} (AUC = {roc_auc[label]:.2f})', **next(cycle_iterator), alpha=0.8)

    else:
        # Single line ROC curve (binary classification)
        label = list(fpr.keys())[0]
        ax.plot(fpr[label], tpr[label], label=f'ROC curve (AUC = {roc_auc[label]:.2f})', color='darkgreen', ls='--')
    ...
    fig = ax.get_figure()
    fig.subplots_adjust(right=0.70)  # Leave room on the right for legend box outside the figure
    return ax
...
def _get_color_wheel(N: int) -> cycle:
    ...
    combined_cycle =  cycler(linestyle=linestyles) * cycler(color=colors) # First color, then ls
