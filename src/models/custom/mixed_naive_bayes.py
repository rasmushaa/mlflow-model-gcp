...
class MNB():
    ...
    def fit(self, features: np.array, y: np.array):
        ...
        self._validate_input(features, y)
    ...
    def _input_pandas_to_numpy(self, df: pd.DataFrame):
        pass


    def _validate_input(self, features: np.array, y: np.array) -> None:
        ''' Validate the user input

        Code will stop for assert Raise,
        if not passed.
        ...
        y: np.array
            Targget variable
        '''
        assert (features.shape[0] == y.shape[0]), 'Features X and Target y shapes must be the same row count'    
        ...
        assert features.ndim == 2, 'String column shape must be a 2 dimensional matrix'
