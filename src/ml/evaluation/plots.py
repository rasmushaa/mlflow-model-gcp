...
def plot_roc_auc_curves(roc_auc_metrs: list) -> plt.Figure:

    n_metrics = len(roc_auc_metrs)
    n_cols = max(
        1, min(n_metrics, 5)
    )  # from 1 to 5 columns, depending on number of metrics
    n_rows = (
        n_metrics + n_cols - 1
    ...
    for i, metric in enumerate(roc_auc_metrs):

        values = metric.values()  # list of roc_auc values
        fprs = metric.metadata["fpr"]  # list of fpr arrays
        tprs = metric.metadata["tpr"]

        # Plot each ROC curve fold
        for fold, (fpr, tpr) in enumerate(zip(fprs, tprs)):
            label = (
                rf"ROC curve ($\overline{{AUC}}$ = {np.mean(values):.2f})"
                if fold == 0
                else None
            )
            ax[i].plot(
        ...
        # Figure settings
        ax[i].plot([0, 1], [0, 1], ls="--", color="gray", alpha=0.2)
